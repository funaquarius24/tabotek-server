...
async function seedDatabase(force = false) {
    ...
    try {
        ...
        await db.collection('categories').updateMany({ slug: { $in: ['phones', 'laptops', 'tablets', 'wearables'] } }, { $set: { parentCategory: reviewsCategoryId } });
        console.log('✅ Reviews subcategories linked');
        // Link Tech Tutorials subcategories
        await db.collection('categories').updateMany({ slug: { $in: ['programming', 'machine-learning', 'data-science-analytics', 'web-development', 'devops'] } }, { $set: { parentCategory: techTutorialsCategoryId } });
        console.log('✅ Tech Tutorials subcategories linked');
        // Link Life Skills subcategories
        await db.collection('categories').updateMany({ slug: { $in: ['tailoring', 'hair-dressing', 'ironing', 'cooking', 'home-maintenance'] } }, { $set: { parentCategory: lifeSkillsCategoryId } });
        console.log('✅ Life Skills subcategories linked');
        // Link GeoPolitics subcategories
        await db.collection('categories').updateMany({ slug: { $in: ['geopolitics-news', 'climate-changes', 'green-energy'] } }, { $set: { parentCategory: geopoliticsCategoryId } });
        console.log('✅ GeoPolitics subcategories linked');
        // Link Blog subcategories
        await db.collection('categories').updateMany({ slug: { $in: ['latest-articles', 'tech-news', 'guides'] } }, { $set: { parentCategory: categoryIds['blog'] } });
