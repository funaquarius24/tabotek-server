...
articleRouter.get('/:slug', async (req, res) => {
    try {
        ...
        await db.collection('articles').updateOne({ _id: article._id }, { $inc: { views: 1 } });
...
articleRouter.put('/:slug', async (req, res) => {
    try {
        ...
        const result = await db.collection('articles').updateOne({ slug }, { $set: updateData });
...
articleRouter.post('/:slug/vote', async (req, res) => {
    try {
        const { slug } = req.params;
