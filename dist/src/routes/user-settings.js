...
userSettingsRouter.get('/settings', async (req, res) => {
    try {
...
userSettingsRouter.put('/settings', async (req, res) => {
    try {
        const userId = req.cookies?.user_id;
        if (!userId || !ObjectId.isValid(userId)) {
            res.status(401).json({ error: 'Not authenticated' });
        ...
        const { db } = await connectToDatabase();
        const now = new Date();
        await db.collection('user_settings').updateOne({ userId: new ObjectId(userId) }, {
            $set: {
                settings,
                updatedAt: now,
            },
