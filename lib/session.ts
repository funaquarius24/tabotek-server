...
function parseSessionCookie(cookieValue: string): string | null {
  try {
...
export async function getServerSession(): Promise<SessionUser | null> {
  ...
  const sessions = db.collection<{ _id: string; expires: Date; session: { user?: SessionUser } }>('sessions');
  const sessionDoc = await sessions.findOne({ _id: sessionId });

  if (!sessionDoc || sessionDoc.expires < new Date()) return null;
